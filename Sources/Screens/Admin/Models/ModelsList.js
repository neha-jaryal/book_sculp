...
import { getAllTalentsList, deleteUser } from "../../../Redux/Services/OtherServices";
...
export const ModelsList = ({ navigation }) => {
  ...
  const getTalents = async () => {
    setLoading(true);
    const res = await dispatch(getAllTalentsList());
    setLoading(false);
    if (res?.status === 200) {
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      ...
      >
        ...
        {loading ? (
          <Skeletoning width={imageSize} height={imageSize + 30} horizontal={0} />
        ) : (
          <FastImage
        ...
        <View style={{ padding: 8 }}>
          <TextComponent
            text={item?.post_meta_details?.display_name || "User"}
            size={Sizes.m}
            fontWeight="500"
            numberOfLines={1}
          />
          ...
          renderItem={({ item, index }) =>
            loading ? (
              ...
              <TouchableOpacity
                ...
                onPress={() => getModelDetails(item)}
              >
                {subscriptionProIds.includes(item?.post_meta_details?.subscription_pro_id) && (
                  <View style={styling.crownIcon}>
                    <MaterialCommunityIcons
                      name="crown"
                      color={
                        item?.post_meta_details?.subscription_pro_id === 103 ||
                        item?.post_meta_details?.subscription_pro_id === 105
                          ? Colors.orange
                          : Colors.yellow
                      }
                      size={30}
