...
export const EditSubmitProposal = ({ route, navigation }) => {
  const { jobDetail, proposalId, proposed_amount, route_name } = route?.params;
  ...
  useEffect(() => {
    if (route_name == routeName?.EDIT_SUBMIT_PROPOSAL) {
      setTotalAmount(proposed_amount);
    } else {
  ...
  const SubmitProposal = async () => {
    ...
    let startDate = moment(jobDetail?.post_meta_details?.starting_date).format(
      "DD/MM/YYYY"
    );
    ...
    let body = {
      ...
      _amt_provided: jobDetail?.post_meta_details?.amount_of_per_diem_provided,
  ...
  return (
    <>
      <Header text={"Edit Proposal"} navigation={navigation} />
      <ScrollView>
        <View style={Styles?.container}>
          ...
          <View style={styling?.employeeDetailView}>
            ...
            <TextComponent
              ...
              color={Colors?.black}
            />
            <Image source={Images?.watchIcon} style={styling?.imageIconView} />
            <TextComponent
          ...
          {showDetails ? (
            <>
              ...
              <View style={{ marginHorizontal: 10 }}>
                <TextComponent
                  text={`$${
                    parseFloat(projectCost?.grandTotal) - parseFloat(serviceFee)
                  }`}
                  size={Sizes?.l}
                  color={Colors?.green}
                />
